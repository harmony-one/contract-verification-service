...
export function verifyByteCode(deployedByteCode, compiledByteCode, solidityVersion) {
  try {
    const compiled = splitByteCode(compiledByteCode, solidityVersion);
    const deployed = splitByteCode(deployedByteCode, solidityVersion);
    return Buffer.compare(compiled, deployed) === 0;
  } catch (e) {
    throw 'Buffer Error. Invalid bytecode';
  }
}
