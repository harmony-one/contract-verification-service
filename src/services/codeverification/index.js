...
const migration = (val) => {
  const str = `const Migrations = artifacts.require("${val}");module.exports = function(deployer) {deployer.deploy(Migrations);};`;
  return str;
};
...
async function codeVerification({
  ...
  truffle.on("close", async (code) => {
    ...
    const compile = new Promise((resolve, rejects) => {
      child_process.exec(
    ...
    const check = new Promise((resolve, rejects) => {
      ...
      if (
        bytecodeContract.substring(0, bytecodeContract.length - 64) ===
        bytecode.deployedBytecode.substring(0, bytecodeContract.length - 64)
      ) {
        {
          console.log("same");
          resolve(true);
